...
describe('ShareLinkSettingsComponent', () => {
  ...
  it('should update share link property successfully', async () => {
    ...
    expect(mockApiService.share.updateShareLink).toHaveBeenCalled();
  ...
  it('should generate share link successfully', async () => {
    const { instance } = await shallow.render({
      bind: { shareItem: {} as any },
    });
    await instance.generateShareLink();

    expect(mockApiService.share.generateShareLink).toHaveBeenCalled();
    expect(instance.shareLink.shareUrl).toBe('http://test/link');
  });

  it('should remove share link successfully', async () => {
    ...
    expect(mockApiService.share.removeShareLink).toHaveBeenCalled();
    expect(instance.shareLink).toBeNull();
  });
