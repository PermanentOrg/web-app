...
export class EditDateTimeMappingService {
	...
	static parseEdtf(edtf: string): EditDateModel | null {
		...
		if (timePart) {
			...
			if (zMatch) {
				const tzParts = zMatch[1].match(/^([+-])(\d{1,2})(?::(\d{2}))?$/);
				if (tzParts) {
			...
			// Extract timezone from bare ISO offset (e.g. "14:30:00-05:00")
			if (!timezoneOffset) {
				const bareMatch = timePart.match(/([+-]\d{2}:\d{2})$/);
				if (bareMatch) {
					timezoneOffset = `GMT${bareMatch[1]}`;
					timezoneName =
						EditDateTimeMappingService.offsetToAbbreviation(timezoneOffset);
				}
			}

			// Strip timezone info for clean time parsing
			const cleanTimePart = timePart
				.replace(/Z.*$/, '')
				.replace(/[+-]\d{2}:\d{2}$/, '');
			...
			seconds = (timeSegments[2] || '').replace(/\.\d+$/, '');
		}
	...
	static buildDisplayDT(date: DateObject, time: TimeObject): string {
		...
		const tzSuffix = EditDateTimeMappingService.buildTzSuffix(
			time.timezoneOffset,
		);
