...
describe('LegacyContactDisplayComponent', () => {
	beforeEach(() => {
		...
		return MockBuilder(LegacyContactDisplayComponent, DummyModule)
			.provide({
				provide: ApiService,
	...
	it('should list legacy contact information', async () => {
		...
		expect(ngMocks.findAll('.legacy-contact-name')[0].nativeElement.innerText).toContain(
			'Test User',
		);

		expect(ngMocks.findAll('.legacy-contact-email').length).toBe(1);
		expect(ngMocks.findAll('.legacy-contact-email')[0].nativeElement.innerText).toContain(
			'test@example.com',
		);
