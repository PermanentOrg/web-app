...
import { fakeAsync, flush, tick, flushMicrotasks, TestBed } from '@angular/core/testing';
...
describe('DirectiveDisplayComponent', () => {
	...
	beforeEach(() => {
		...
		return MockBuilder(DirectiveDisplayComponent, DummyModule)
			.keep(AccountService)
			.keep(ApiService)
			.provide({
	...
	it('should fetch directive info from API', fakeAsync(() => {
		...
		expect(ngMocks.findAll('.archive-steward-note')[0].nativeElement.innerText).toContain(
			'Unit Testing!',
		);

		expect(ngMocks.findAll('.archive-steward-email')[0].nativeElement.innerText).toContain(
			'test@example.com',
		);
	}));
	...
	it('should show the "No Plan" warning if the user does not have a legacy contact', async () => {
		...
		expect(fixture.nativeElement.querySelectorAll('.no-plan-warning').length).toBe(1);
	...
	it('should be able to handle API errors when fetching Legacy Contact', async () => {
		...
		expect(fixture.nativeElement.querySelectorAll('.archive-steward-table').length).toBe(0);
		expect(fixture.nativeElement.querySelector('button').disabled).toBeTruthy();
	});
	...
	it('should say "Assign" for new directive', fakeAsync(() => {
		...
		expect(ngMocks.find('button').nativeElement.innerText).not.toContain('Edit');
	}));
