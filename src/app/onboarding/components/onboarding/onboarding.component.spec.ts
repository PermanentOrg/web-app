...
describe('OnboardingComponent #onboarding', () => {
	...
	it('should load the create new archive screen as default', async () => {
		...
		expect(fixture.nativeElement.querySelectorAll('pr-create-new-archive').length).toBe(1);
	});
	...
	it('can change screens', async () => {
		...
		expect(fixture.nativeElement.querySelectorAll('pr-create-new-archive').length).toBe(1);
		expect(fixture.nativeElement.querySelectorAll('pr-welcome-screen').length).toBe(0);
	});
	...
	it('stores an accepted archive invitation', async () => {
		...
		expect(fixture.nativeElement.querySelectorAll('pr-welcome-screen').length).toBe(1);
		ngMocks.find('pr-welcome-screen').triggerEventHandler(
			'selectInvitation',
