...
describe('UserChecklistComponent', () => {
	...
	beforeEach(() => {
		...
		return MockBuilder(UserChecklistComponent, UserChecklistModule).provide({
	...
	it('should list all tasks received from the API', fakeAsync(() => {
		...
		expect(fixture.point.nativeElement.innerText).toContain('Write a unit test');
	...
	it('should mark completed status on completed tasks', fakeAsync(() => {
		...
		expect(ngMocks.findAll('.task .fake-checkbox.checked').length).toBeGreaterThan(0);
	}));
