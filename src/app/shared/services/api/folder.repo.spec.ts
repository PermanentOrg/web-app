...
describe('Folder repo', () => {
	...
	describe('resolveFolderId', () => {
		...
		it('should return a v1-shaped error response when /folder/get cannot resolve a folderId', async () => {
			const folderVO = new FolderVO({
				archiveNbr: '0001-0001',
				folder_linkId: 123,
			});

			// The legacy lookup returns no Results, so getFolderVO() is null.
			httpSpy.sendRequestPromise.and.resolveTo(
				new FolderResponse({ Results: [] }),
			);
			...
			expect(result.isSuccessful).toBeFalsy();
			expect(result.getMessage()).toBe(
				'Unable to resolve folderId via /folder/get',
			);
