...
const convertStelaFolderToFolderVO = (stelaFolder: StelaFolder): FolderVO => {
	...
	return new FolderVO({
		...
		folder_linkId: Number.parseInt(stelaFolder.folderLinkId, 10),
...
export class FolderRepo extends BaseRepo {
	...
	private async resolveFolderId(folderVO: FolderVO): Promise<FolderVO> {
		...
		if (!resolvedFolder?.folderId) {
			throw new Error('Unable to resolve folderId via /folder/get');
		}
	...
	public async getWithChildren(
		...
		try {
			...
			// Changing all the error handlers would be error prone
