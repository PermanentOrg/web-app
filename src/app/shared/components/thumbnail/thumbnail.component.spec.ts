...
describe('ThumbnailComponent', () => {
	...
	beforeEach(() => {
		...
		thumbnailComponent = fixture.debugElement.children[0].children[0].componentInstance;
	});

	function setItem(item: ItemVO, size: number = 200, maxWidth?: number) {
		hostComponent.item = item.isFolder ? new FolderVO(item) : new RecordVO(item);
		...
		const thumbnailElement = fixture.debugElement.children[0].children[0].nativeElement;
	...
	it('should use reset when changing records', async () => {
		...
		expect(thumbnailComponent.getTargetThumbWidth()).toEqual(500);
		expect(thumbnailComponent.getCurrentThumbUrl()).toEqual(fullItem2.thumbURL500);
	});
