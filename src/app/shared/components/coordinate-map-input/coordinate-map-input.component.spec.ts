...
describe('CoordinateMapInputComponent', () => {
	...
	describe('with coordinates given', () => {
		...
		it('should report itself valid', () => {
			expect(host.validity).toEqual([true]);
		});
	});

	describe('with coordinates that name no place on earth', () => {
		beforeEach(async () => {
			await setUp({ latitude: 200, longitude: 0 });
		});

		it('should report itself invalid rather than let them be confirmed', () => {
			expect(host.validity).toEqual([false]);
		});

		it('should still show them, so they can be corrected', () => {
			expect(getField().value).not.toBe('');
		});

		it('should drop no pin on a position Google Maps cannot hold', () => {
			expect(host.input.markerPosition()).toBeNull();
		});

		it('should leave the map on the whole country', () => {
			expect(host.input.mapOptions.zoom).toBe(4);
		});

		it('should open on the first real place typed instead', () => {
	...
	describe('clicking the map', () => {
		...
		it('should keep the view the click was made in, panning to pairs typed after it', () => {
			...
			host.input.onMapClick(mapClickAt(LISBON.latitude, LISBON.longitude));
			fixture.detectChanges();
			typeInField('-33.8688, 151.2093');

			expect(host.input.mapOptions.zoom).toBe(4);
			expect(panTo).toHaveBeenCalledOnceWith({
				lat: -33.8688,
				lng: 151.2093,
			});
		});
	...
	describe('typing into the field', () => {
		...
		it('should open on the first pair typed', () => {
			typeInField('38.70786, -9.400139');
		...
		it('should follow later pairs with the map', () => {
			typeInField('38.70786, -9.400139');
			typeInField('-33.8688, 151.2093');
		...
		it('should open on a pair typed before the Maps API has loaded', () => {
			host.input.map = { panTo } as unknown as typeof host.input.map;
		...
		it('should open on the latest pair typed before the Maps API has loaded', () => {
			host.input.map = { panTo } as unknown as typeof host.input.map;
