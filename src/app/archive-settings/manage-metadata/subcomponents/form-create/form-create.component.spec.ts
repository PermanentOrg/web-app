...
describe('FormCreateComponent', () => {
	beforeEach(() => {
		return MockBuilder(FormCreateComponent, ManageMetadataModule)
			.keep(FormsModule)
			.keep(A11yModule);
	});

	function defaultRender(c: (tagName: string) => Promise<void> = async () => {}) {
		return MockRender(
	...
	it('should use specified placeholder text', () => {
		...
		expect(ngMocks.find('input').nativeElement.placeholder).toBe('Add New Test');
	});
