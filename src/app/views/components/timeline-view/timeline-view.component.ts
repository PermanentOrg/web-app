...
export class TimelineViewComponent implements OnInit, AfterViewInit, OnDestroy {
	...
	constructor(
		...
		({ route: { snapshot: { data: { currentFolder: this.timelineRootFolder } } } } = this);
	...
	onFolderChange() {
		...
		({ dataService: { currentFolder: this.timelineRootFolder } } = this);
	...
	initTimeline() {
		const { timelineElemRef: { nativeElement: container } } = this;
	...
	findBestTimezone() {
		const counts = countBy(
			...
			(i: ItemVO) => {
				const { TimezoneVO: { timeZoneId: id } } = i;
	...
	protected addPixelMargin(): void {
		...
		for (const uuid in this.timeline.itemSet.items) {
			if (uuid.includes('-')) {
				// @ts-ignore
				const { timeline: { itemSet: { items: { [uuid]: item } } } } = this;
