import {
  ComponentFixture,
  TestBed,
  fakeAsync,
  tick,
} from '@angular/core/testing';
import { AccountService } from '@shared/services/account/account.service';
import { ApiService } from '@shared/services/api/api.service';
import { MessageService } from '@shared/services/message/message.service';
import { ActivatedRoute } from '@angular/router';
import { of, throwError } from 'rxjs';
import { DIALOG_DATA, DialogRef } from '@angular/cdk/dialog';
import { AccountResponse } from '@shared/services/api/index.repo';
import { NO_ERRORS_SCHEMA } from '@angular/core';
import {
  AccountSettingsDialogComponent,
  SettingsTab,
} from './account-settings-dialog.component';

describe('AccountSettingsDialogComponent', () => {
  let component: AccountSettingsDialogComponent;
  let fixture: ComponentFixture<AccountSettingsDialogComponent>;

  let mockDialogRef: jasmine.SpyObj<DialogRef>;
  let mockAccountService: jasmine.SpyObj<AccountService>;
  let mockApiService: jasmine.SpyObj<ApiService>;
  let mockMessageService: jasmine.SpyObj<MessageService>;

  beforeEach(async () => {
    mockDialogRef = jasmine.createSpyObj('DialogRef', ['close']);
    mockAccountService = jasmine.createSpyObj('AccountService', [
      'getAccount',
      'logOut',
      'isEmailOrPhoneUnverified',
    ]);
    mockApiService = jasmine.createSpyObj('ApiService', [], {
      account: { delete: jasmine.createSpy() },
    });
    mockMessageService = jasmine.createSpyObj('MessageService', ['showError']);

    await TestBed.configureTestingModule({
      declarations: [AccountSettingsDialogComponent],
      providers: [
        { provide: DIALOG_DATA, useValue: {} },
        { provide: DialogRef, useValue: mockDialogRef },
        { provide: AccountService, useValue: mockAccountService },
        { provide: ApiService, useValue: mockApiService },
        { provide: MessageService, useValue: mockMessageService },
        {
          provide: ActivatedRoute,
          useValue: { snapshot: { fragment: null } },
        },
      ],
      schemas: [NO_ERRORS_SCHEMA],
    }).compileComponents();

    fixture = TestBed.createComponent(AccountSettingsDialogComponent);
    component = fixture.componentInstance;
    fixture.detectChanges();
  });

  it('should create component', () => {
    expect(component).toBeTruthy();
  });

  it('should initialize with default activeTab as "email-phone"', () => {
    expect(component.activeTab).toBe('email-phone');
  });

  it('should set activeTab from DIALOG_DATA if provided', () => {
    const data = { tab: 'security' };
    TestBed.overrideProvider(DIALOG_DATA, { useValue: data });
    const fixture2 = TestBed.createComponent(AccountSettingsDialogComponent);
    const comp2 = fixture2.componentInstance;
    fixture2.detectChanges();

    expect(comp2.activeTab).toBe('security');
  });

  it('should set activeTab from route fragment if valid', () => {
    TestBed.overrideProvider(ActivatedRoute, {
      useValue: { snapshot: { fragment: 'delete' } },
    });
    const fixture3 = TestBed.createComponent(AccountSettingsDialogComponent);
    const comp3 = fixture3.componentInstance;
    fixture3.detectChanges();

    expect(comp3.activeTab).toBe('delete');
  });

  it('should switch tab when setTab is called', () => {
    component.setTab('notification');

    expect(component.activeTab).toBe('notification');
  });

  it('should call dialogRef.close() on onDoneClick()', () => {
    component.onDoneClick();

    expect(mockDialogRef.close).toHaveBeenCalled();
  });

  describe('onDeleteAccountConfirm()', () => {
    beforeEach(() => {
      component.deleteVerify = component.verifyText;
      mockAccountService.getAccount.and.returnValue('account-id');
    });

    it('should not proceed if deleteVerify does not match verifyText', async () => {
      component.deleteVerify = 'WRONG';
      await component.onDeleteAccountConfirm();

      expect(mockApiService.account.delete).not.toHaveBeenCalled();
    });

    it('should call api.account.delete and accountService.logOut on success', fakeAsync(async () => {
      (mockApiService.account.delete as jasmine.Spy).and.resolveTo({});
      mockAccountService.logOut.and.resolveTo();

      await component.onDeleteAccountConfirm();
      tick();

      expect(mockApiService.account.delete).toHaveBeenCalledWith('account-id');
      expect(mockAccountService.logOut).toHaveBeenCalled();
    }));

    it('should show error message when AccountResponse error is thrown', fakeAsync(async () => {
      const err = new AccountResponse();
      spyOn(err, 'getMessage').and.returnValue('Error!');
      (mockApiService.account.delete as jasmine.Spy).and.rejectWith(err);

      await component.onDeleteAccountConfirm();
      tick();

      expect(mockMessageService.showError).toHaveBeenCalledWith({
        message: 'Error!',
        translate: true,
      });
    }));

    it('should show generic error message when unknown error is thrown', fakeAsync(async () => {
      (mockApiService.account.delete as jasmine.Spy).and.rejectWith(
        new Error('fail'),
      );

      await component.onDeleteAccountConfirm();
      tick();

      expect(mockMessageService.showError).toHaveBeenCalledWith({
        message: 'There was an error deleting the account.',
        translate: false,
      });
    }));

    it('should reset waiting flag after completion', fakeAsync(async () => {
      (mockApiService.account.delete as jasmine.Spy).and.resolveTo({});
      mockAccountService.logOut.and.resolveTo();

      await component.onDeleteAccountConfirm();
      tick();

      expect(component.waiting).toBeFalse();
    }));
  });
});
