...
describe('TwoFactorAuthComponent', () => {
  ...
  beforeEach(async () => {
    ...
    it('should update validators when method changes', async () => {
      ...
      const result = control.validator({ value: '(123) 456 - 7890' } as AbstractControl);
      expect(result).toBeNull();  // Indicates valid input
    });

    it('should remove method and update form state', async () => {
      const { instance } = await shallow.render();
      const method = { id: 'email', method: 'email', value: 'user@example.com' };
    ...
    it('should display methods correctly in the table', async () => {
      ...
      expect(methodRows[0].nativeElement.textContent).toContain('janedoe@example.com');
